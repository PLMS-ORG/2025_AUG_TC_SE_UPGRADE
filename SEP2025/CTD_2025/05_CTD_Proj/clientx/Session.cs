...
namespace Teamcenter.ClientX
{
    ...
    public class Session
    {
         ...
         *
         */
        public User login(String user, String password,String group,String role,String discriminator)
        {
            ...
            {
                Log.log.write(Log.logType.ERROR, "Login failed.Session service is null.");
                return null;
            }
                ...
                if (resp.ServiceData.sizeOfPartialErrors() > 0)
                {
                    if (resp.ServiceData.sizeOfPartialErrors() > 0)
                    {
                        ...
                        for (int i = 0; i < resp.ServiceData.sizeOfPartialErrors(); i++)
                        {
                            ...
                            for (int j = 0; j < resp.ServiceData.GetPartialError(i).Messages.Length; j++)
                            {
                                error = error + "." + resp.ServiceData.GetPartialError(i).Messages[j];
                            }
                        }

                        Log.log.write(Log.logType.ERROR, "Login failed." + error);
            ...
            {
                Log.log.writeException( e, "login");
            ...
            {
                Log.log.writeException(ex, "login");
            }
            return null;
