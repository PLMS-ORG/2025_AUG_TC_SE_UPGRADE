...
namespace _2DAutomaticUpdate
{
    ...
    {
        public static void SearchDraftFile(String searchDrawingsFolder, String logFilePath)
            {
                //String searchDrawingsFolder = System.IO.Path.GetDirectoryName(assemblyFileName);
                Utility.Log("searchDrawingsFolder: " + searchDrawingsFolder, logFilePath);
                ...
                {
                    Utility.Log("SearchDraftFile -  No Draft Files Found ", logFilePath);
                    return;
                }
                ...
                foreach (String draftFile in draftFiles)
                {
                    ...
                    {
                        Utility.Log(draftFile + " is applicable for this assembly", logFilePath);
                        draftFilesTemp.Add(draftFile);
                    }
                    else
                        Utility.Log(draftFile + " is not applicable for this assembly", logFilePath);
                }
                ...
                foreach (String draftFile in draftFiles)
                {
                    try
                    {
                        Utility.Log("findOutOfDateDrawing: " + System.DateTime.Now.ToString(), logFilePath);
                    ...
                    {
                        Utility.Log("SolidEdgeUpdateView, findOutOfDateDrawing: " + ex.Message, logFilePath);
                        return;
                    }
                }
            ...
            {
                Utility.Log("findOutOfDateDrawing -  DraftFileFULLPath: " + DraftFileFULLPath, logFilePath);
                ...
                if (objApp == null)
                {
                    Utility.Log("DEBUG -  objApp is NULL : " + DraftFileFULLPath, logFilePath);
                    return false;
                }
                ...
                {
                    Utility.ResetAlerts(objApp, true, logFilePath);
                    Utility.Log("DEBUG -  objDocuments is NULL : " + DraftFileFULLPath, logFilePath);
                    return false;
                }
                ...
                {
                    Utility.Log("DEBUG - OleMessageFilter Register : " + DraftFileFULLPath, logFilePath);
                ...
                try
                {
                    if (System.IO.File.Exists(DraftFileFULLPath) == true)
                    {
                        ...
                        if (objDraftDocument.ReadOnly == true)
                        {
                            ...
                            FileInfo f = new FileInfo(DraftFileFULLPath);
                            f.IsReadOnly = false;
                ...
                {
                    Utility.Log("DEBUG - No partsList in the Draft File : " + DraftFileFULLPath, logFilePath);
                }

                Utility.Log("------ PROCESS SHEETS ------", logFilePath);
                    ...
                    foreach (SolidEdgeDraft.Sheet sheet in sections.WorkingSection.Sheets)
                    {
                        ...
                        if (views == null)
                        {
                            ...
                            Utility.Log("No Views in the Sheet: " + sheet.Name, logFilePath);
                            continue;
                        }
                        Utility.Log("Sheet: " + sheet.Name, logFilePath);
                        foreach (SolidEdgeDraft.DrawingView vw in views)
                        {
                            Utility.Log("View Name " + vw.Name + "::::" + " View UpToDateStatus " + vw.IsUpToDate, logFilePath);
                            if (vw.IsUpToDate == false)
                            {
                                //Utility.Log("DraftFileName: " + DraftFileFULLPath + "  Sheet: " + sheet.Name +
                                //" View Name " + vw.Name + " View UpToDateStatus " + vw.IsUpToDate, outPutFilePath);
                                //Utility.Log("DraftFileName: " + SolidEdgeDraftFilePath + "  Sheet: " + sheet.Name                            , outPutFilePath);

                                Utility.Log("ForceUpdate: " + sheet.Name, logFilePath);
                                try
                                {
                                ...
                                {
                                    Utility.Log("ForceUpdate: " + "Exception: " + ex.Message, logFilePath);
                                }
                            }
                ...
                {
                    Utility.ResetAlerts(objApp, true, logFilePath);
                    Utility.Log("Replacing Exception: ", logFilePath);
                    throw new System.Exception("No active document.");
                }

                
            
            // ==== Autoscaling Implementation in Drawing Views == Murali - Implemented on February 16 2022
            try
            {
                foreach (SheetGroup sheetGroup in (SheetGroups)objDraftDocument.SheetGroups)
                {
                    Utility.Log("DEBUG - Found Sheet Group: ", logFilePath);
                    foreach (Sheet sheet1 in sheetGroup.Sheets)
                    {
                        Utility.Log(string.Concat("DEBUG - Found Sheet : ", sheet1.Name), logFilePath);
                        foreach (DrawingView drawingView1 in sheet1.DrawingViews)
                        {
                            ...
                            Utility.Log(string.Concat("DEBUG - Found Sheet : ", sheet1.Name), logFilePath);
                            Utility.Log(string.Concat("DEBUG - description : ", description), logFilePath);
                            if ((description == null || description.Equals("") == true))
                            {
                                try
                                {
                                    Utility.Log(string.Concat("View ForceUpdate : ", drawingView1.Name), logFilePath);
                                    drawingView1.ForceUpdate();
                                }
                                catch (Exception ex)
                                {
                                    Utility.Log(string.Concat("ForceUpdate: Exception: ", ex.Message), logFilePath);
                                }
                            }
                            else
                            {
                                ...
                                if ((strArrays != null && strArrays.Length == 2))
                                {
                                    ...
                                    try
                                    {
                                        Utility.Log(string.Concat("DEBUG - strMaxScaleLength : ", scaleLength), logFilePath);
                                        double.TryParse(scaleLength, out num);
                                    }
                                    catch (Exception ex11)
                                    {
                                       
                                        Utility.Log(string.Concat("DEBUG - Exception : ", ex11.Message), logFilePath);
                                        continue;
                                    }
                                    if (num != 0)
                                    {
                                        Utility.Log(string.Concat("drawingView.Name: ", drawingView1.Name), logFilePath);
                                        if ((str1 !=null && str1.Equals("") == false))
                                        {
                                            if (str1.Equals("AUTOSCALE", StringComparison.OrdinalIgnoreCase))
                                            {
                                                Utility.Log(string.Concat("maxScaleLength : ", num.ToString()), logFilePath);
                                                Utility.Log(string.Concat("maxLength : ", length.ToString()), logFilePath);
                                                if (length != 0)
                                                {
                                                    drawingView1.ScaleFactor = (num / length);
                                                    double scaleFactor = drawingView1.ScaleFactor;
                                                    Utility.Log(string.Concat("ScaleFactor : ", scaleFactor.ToString()), logFilePath);
                                                }
                                                else
                                                {
                                                    Utility.Log("DEBUG - Unable to perform autoScale : " + drawingView1.Name, logFilePath);
                                                    continue;
                                                   
                                                }
                                            } else
                                            {
                                                Utility.Log("DEBUG - autoScaleOption is not enabled : " + drawingView1.Name, logFilePath);
                                                continue;
                                            }
                                            try
                                            {
                                            ...
                                            catch (Exception ex1)
                                            {
                                                ...
                                                Utility.Log(string.Concat("ForceUpdate: Exception: ", ex1.Message), logFilePath);
                                            }
                                        }
                                        else
                                        {
                                            Utility.Log("DEBUG - autoScaleOption is Empty : ", logFilePath);
                                            continue;
                                        }
                                    }
                                    else
                                    {
                                        Utility.Log("DEBUG - Scale length is zero : " + drawingView1.Name, logFilePath);
                                        continue;
                                    }
                                } else
                                {
                                    Utility.Log("DEBUG - autoScaleOption is not enabled for view : " + drawingView1.Name, logFilePath);
                                    continue;
            ...
            {
                Utility.Log(string.Concat("DEBUG - Exception : ", ex3.Message), logFilePath);
            ...
            try
            {
                Utility.Log("DEBUG - ClearAllDimensionTrackerEntries", logFilePath);
        ...
        private static void updatePartsList(PartsLists partsLists, string logFilePath)
        {
            foreach (SolidEdgeDraft.PartsList partsList in partsLists)
            {
                ...
                {
                    Utility.Log("DEBUG - partsList Update: ", logFilePath);
                    partsList.Update();
                }
                else
                {
                    Utility.Log("DEBUG - partsList is already Up to Date.. ", logFilePath);
                }
            }
        }
