...
/*
 * MURALI - 28-03-2023 - Fix for Issue in PSM file processing
 **/
    ...
    class SolidEdgeInterface
    {
            ...
            try
            {
                ...
                if (objAssemblyDocument != null)
                {
                    ...
                    for (int i = 1; i <= occurrences.Count; i++)
                    {
                        ...
                        if (occurrence == null)
                        {
                            Utlity.Log("occurrence is null..", logFilePath);
                            continue;
                        }
                        ...
                        {
                            Utlity.Log("occurenceName is empty..", logFilePath);
                            continue;
                        }
                        Utlity.Log("occurenceName is: "+ occurenceName, logFilePath);
                        ...
                        //02-12-2024 - Murali - ExcelSyncDialog.listOfFileNamesInSession some times misses child components - Reason not known.
                        ...
                        if (ExcelSyncDialog.listOfFileNamesInSession.Contains(ocurenceFileNameWithoutPath) == false)
                        {
                            Utlity.Log("listOfFileNamesInSession Adding Missed ocurence--" + ocurenceFileNameWithoutPath, logFilePath);
                            ExcelSyncDialog.listOfFileNamesInSession.Add(ocurenceFileNameWithoutPath);
                        }
                        ...
                        if (occurrence.OccurrenceDocument is SolidEdgePart.PartDocument)
                        {
                            ...
                            if (partDoc == null)
                            {
                                Utlity.Log("partDoc is null..", logFilePath);
                                continue;
                            }
                            ...
                            if (partDoc.ReadOnly == true)
                            {
                                ...
                                if (WriteAccess == false)
                                {
                                    ...
                                    Utlity.Log("Could not get WriteAccess to--" + partDoc.FullName, logFilePath);
                                    //continue;
                        ...
                        else if (occurrence.OccurrenceDocument is SolidEdgePart.SheetMetalDocument)
                        {
                            ...
                            {
                                Utlity.Log("sheetMetalDoc is null..", logFilePath);
                                continue;
                            }
                            ...
                            if (sheetMetalDoc.ReadOnly == true)
                            {
                                ...
                                {
                                    //Utility.ResetAlerts(objApp, false, logFilePath);
                                    Utlity.Log("Could not get WriteAccess to--" + sheetMetalDoc.FullName, logFilePath);
                                    //continue;
                        ...
                        else if (occurrence.OccurrenceDocument is SolidEdgeAssembly.AssemblyDocument)
                        {
                            ...
                            if (assemDoc == null)
                            {
                                Utlity.Log("assemDoc is null..", logFilePath);
                                continue;
                            }
                            ...
                            if (assemDoc.ReadOnly == true)
                            {
                                ...
                                {
                                    //Utility.ResetAlerts(objApp, false, logFilePath);
                                    Utlity.Log("Could not get WriteAccess to--" + assemDoc.FullName, logFilePath);
                                    //continue;
        ...
        public static void traverseAssemblyToSetVariables(SolidEdgeAssembly.AssemblyDocument assemDoc, SolidEdgeFramework.Variables variables, String logFilePath)
        {
            ...
            for (int i = 1; i <= occurrences.Count; i++)
            {
                ...
                if (occurrence == null)
                {
                    Utlity.Log("occurrence is null..continue", logFilePath);
                    continue;
                }
                String occurenceName = occurrence.Name;
                String[] occArr = occurenceName.Split(':');
               ...
               // Utlity.Log("occurenceName--" + occurenceName, logFilePath);
                ...
                if (occurrence.OccurrenceDocument is SolidEdgePart.PartDocument)
                {
                    ...
                    if (partDoc == null)
                    {
                        Utlity.Log("partDoc is null..continue", logFilePath);
                        continue;
                    }
                ...
                else if (occurrence.OccurrenceDocument is SolidEdgePart.SheetMetalDocument)
                {
                    ...
                    if (sheetMetalDoc == null)
                    {
                        Utlity.Log("sheetMetalDoc is null..continue", logFilePath);
                        continue;
                    }
                    ...
                    if(f == null)
                    {
                        Utlity.Log("FileInfo is null..continue", logFilePath);
                        continue;
                    }
                    f.IsReadOnly = false;
                    // AVM - 28-03-2023 : Fixed Issue for PSM file processing.
                    if (sheetMetalDoc.ReadOnly == true)
                    {
                        bool WriteAccess = false;
                    ...
                    saveSheet(sheetMetalDoc, logFilePath);
                    Utlity.Log("saveSheet completed--" + sheetMetalDoc.FullName, logFilePath);
                }
                else if (occurrence.OccurrenceDocument is SolidEdgeAssembly.AssemblyDocument)
                {
                    ...
                    if (assemDoc1 == null)
                    {
                        Utlity.Log("assemDoc1 is null..continue", logFilePath);
                        continue;
                    }
                    ...
                    if (assemDoc1.ReadOnly == true)
                    {
                        ...
                        {
                            //Utility.ResetAlerts(objApp, false, logFilePath);
                            Utlity.Log("Could not get WriteAccess to--" + assemDoc1.FullName, logFilePath);
                            //continue;
                    ...
                    {
                        Utlity.Log("variables1 is null..skipped traverseAssemblyToSetVariables", logFilePath);
                    }
                    SaveAndCloseAssembly(assemDoc, logFilePath);
                }
        ...
        private static SolidEdgeFramework.Documents OpenDocument(String logFilePath)
        {
            ...
            {
                Utlity.Log("objDocuments object is NULL", logFilePath);
                return null;
        ...
        public static void setVariableInSolidEdge(SolidEdgeFramework.Variables variables, String variableName, String Value, String logFilePath)
        {
            ...
            if (variables == null)
            {
                Utlity.Log("variables object is NULL", logFilePath);
                return;
            }
        ...
        private static void SaveAndCloseAssembly(SolidEdgeAssembly.AssemblyDocument assemblyDoc, String logFilePath)
        {
            try
            {
                ...
                if (assemblyDoc.ReadOnly == true)
                {
                    Utlity.Log("SaveAndCloseAssembly for read only file : " + assemblyDoc.FullName, logFilePath);
                    //#1848|AUG 7,2025
                    Utlity.Log("Trying to save Read only file.....", logFilePath);
                    bool writeaccess = false;
                    assemblyDoc.SeekWriteAccess(out writeaccess);
                    if(assemblyDoc.ReadOnly==false)
                    {
                        Utlity.Log("SaveAndCloseAssembly: " + " Save", logFilePath);
                        assemblyDoc.Save();
                    ...
                    {
                        Utlity.Log("SaveAssembly is failed since Assembly is read only" + assemblyDoc.FullName, logFilePath);
                    }
                }
            }
            catch (Exception ex)
        ...
        private static void savePart(SolidEdgePart.PartDocument partDoc, String logFilePath)
        {
            try
            {
                ...
                if (partDoc.ReadOnly == true)
                {
                    Utlity.Log("savePart for read only file: " + partDoc.FullName, logFilePath);
                    //#1848|AUG 7,2025
                    Utlity.Log("Trying to save Read only file.....", logFilePath);
                    bool writeAccess = false;
                    partDoc.SeekWriteAccess(out writeAccess);
                    if (partDoc.ReadOnly == false)
                    {
                        Utlity.Log("Save part:", partDoc.FullName, logFilePath); 
                        partDoc.Save();
                    }
                    else
                    {
                        Utlity.Log("Savepart failed since part is readonly:" + partDoc.FullName,logFilePath);
                    }
        ...
        private static void saveSheet(SolidEdgePart.SheetMetalDocument sheetMetalDoc, String logFilePath)
        {
            try
            {
                ...
                if (sheetMetalDoc.ReadOnly == true)
                {
                    Utlity.Log("saveSheet for read only file: " + sheetMetalDoc.FullName, logFilePath);
                    // #1848|AUG 7,2025
                    Utlity.Log("Trying to save Read only file.....", logFilePath);
                    bool writeAccess = false;
                    sheetMetalDoc.SeekWriteAccess(out writeAccess);
                    if(sheetMetalDoc.ReadOnly==false)
                    {
                        Utlity.Log("Savepart failed since part is readonly:" + sheetMetalDoc.FullName, logFilePath);
                        sheetMetalDoc.Save();
                    }
                    else
                    {
        ...
        private static void SetVariableData(SolidEdgeFramework.UnitsOfMeasure UOM, string occurenceName, Variable updatedVar, SolidEdgeFramework.Variables variables, String logFilePath, String Option)
        {
            ...
            if (variableList == null || variableList.Count == 0)
            {
                ...
                if (variableList == null || variableList.Count == 0)
                {
                    Utlity.Log(updatedVar.name + " not Found..", logFilePath);
                    //pFindCriterium = "*";
                    //variableList = (SolidEdgeFramework.VariableList)variables.Query(pFindCriterium, NamedBy, VarType, CaseInsensitive);
                    //if (variableList == null || variableList.Count == 0)
                    //{
                ...
                switch (objectType)
                {
                    case SolidEdgeFramework.ObjectType.igDimension:
                        ...
                        if (dimension != null)
                        {
                            ...
                            {
                                //Utlity.Log("dimensionName: " + updatedVar.name, logFilePath);
                                //Utlity.Log("GetValue: " + gvalue, logFilePath);
