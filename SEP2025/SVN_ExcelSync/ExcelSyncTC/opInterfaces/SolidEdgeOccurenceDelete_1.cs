...
namespace ExcelSyncTC.opInterfaces
{
    class SolidEdgeOccurenceDelete_1 : IsolatedTaskProxy
    {
        ...
        public void SolidEdgeOccurrenceDeleteFromExcel(String assemblyFileName, String logFilePath)
        {
            ...
            {
                Utlity.Log("SolidEdgeOccurrenceDeleteFromExcel: " + "Occurrence List Is Empty", logFilePath);
                return;
            }
            ...
            //{
            //    Utlity.Log("SolidEdgeOccurrenceDeleteFromExcel: " + "Remove Component List is Empty", logFilePath);
            //    return;
            //}

            //var ListOfComponentsToDelete = occurenceList.Except(MasterAssemblyList).ToList();

            List<String> ListOfComponentsToDelete = MasterAssemblyReader.getExcludedComponents();
            if (ListOfComponentsToDelete == null || ListOfComponentsToDelete.Count == 0)
            {
                Utlity.Log("SolidEdgeOccurrenceDeleteFromExcel: " + "ListOfComponentsToDelete is Empty", logFilePath);
                return;
            }
            ...
            {
                Utlity.Log("SolidEdgeOccurrenceDeleteFromExcel: " + "ComponentsToDelete is Empty", logFilePath);                
                return;
            }
            var ComponentStringDelete = string.Join( ",", ComponentsToDelete );
            Utlity.Log("Components to be Deleted: " + ComponentStringDelete, logFilePath);
            String SearchAssemblyFileFolder = System.IO.Path.GetDirectoryName(assemblyFileName);
            Utlity.Log("SearchAssemblyFileFolder: " + SearchAssemblyFileFolder, logFilePath);
            ...
            {
                Utlity.Log("SearchAssemblyFileFolder -  No Assembly Files Found ", logFilePath);
                return;
            }
            List<string> asmFilesTemp = new List<string>();
            foreach (String asmFile in AssemblyFiles)
            {
                if (ExcelSyncDialog.listOfFileNamesInSession.Contains(Path.GetFileName(asmFile.ToLower().Trim())))
                {
                    Utlity.Log(asmFile + " is applicable for this assembly", logFilePath);
                    asmFilesTemp.Add(asmFile);
                }
                else
                    Utlity.Log(asmFile + " is not applicable for this assembly", logFilePath);
            }
            ...
            try
            {
                ...
                if (objAssemblyDocument != null)
                {
                    ...
                    for (int i = occurrences.Count ; i >=1; i--)
                    {
                        ...
                        Utlity.Log("occurenceName--" + occurrence.Name, logFilePath);
        ...
        private void SaveAssembly(SolidEdgeAssembly.AssemblyDocument assemblyDoc, String logFilePath)
        {
            try
            {
                ...
                {
                    Utlity.Log("Save: " + assemblyDoc.FullName, logFilePath);
                    assemblyDoc.Save();
                }
        ...
        private void SaveAndCloseAssembly(SolidEdgeAssembly.AssemblyDocument assemblyDoc, String logFilePath)
        {
            try
            {
                ...
                if (assemblyDoc.ReadOnly == false)
                {
                    Utlity.Log("Close: " + assemblyDoc.FullName, logFilePath);
                    assemblyDoc.Close();
                }
        ...
        private void ReadAllOccurrencesFromAssembly(String assemblyFileName, String logFilePath)
        {
            ...
            try
            {
                ...
                Utlity.Log("DEBUG - InputFile is Opened : " + objAssemblyDocument.FullName, logFilePath);
