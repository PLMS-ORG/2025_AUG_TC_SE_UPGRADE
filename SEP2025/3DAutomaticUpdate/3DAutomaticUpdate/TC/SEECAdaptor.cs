...
namespace _3DAutomaticUpdate.TC
{
    class SEECAdaptor
    {
        ...
        private static string bStrUserName;
        ...
        public static void LoginToTeamcenter()
        {
            ...
            objSEEC.ValidateLogin(Program.userName, Program.password, Program.group, Program.role, Program.URL);
        ...
        public static void CheckInSEDocumentsToTeamcenter(string logFilePath, List<string> listOfFileNamesInSession)
        {
            ...
            {
                Utility.Log("CheckInSEDocumentsToTeamcenter, Exception: " + ex.Message, logFilePath);
            }
            ...
            try
            {
                if (parFiles.Length != 0)
                {
                    ...
                    foreach (String partFile in parFiles)
                    {
                        ...
                        Utility.Log(partFile + "...checkout Status...: " + ischeckedout, logFilePath);
            ...
            try
            {
                //UploadExcelTemplateToTeamcenterUsing3DEUpload(logFilePath);
                writeExcelFilePathInTxt(logFilePath);
            }
            catch (Exception ex)
            {
                Utility.Log("Exception while writeExcelFilePathInTxt " + ex.ToString(), logFilePath);
            }
            //objSEEC.DeleteFilesFromCache(ref ppsaAssemFileList);

        }
        public static void writeExcelFilePathInTxt(String logFilePath)
        {
            ...
            String[] XLFiles = Directory.GetFiles(bstrCachePath, "*", SearchOption.AllDirectories)
                                            ...
                                            .ToArray();
            Utility.Log("Number of xlsx files:" + XLFiles.Length.ToString(), logFilePath);
            ...
            foreach (String XLFile in XLFiles)
            {
                ...
                Utility.Log(XLFiles[0] + "...checkout Status...: " + ischeckedout, logFilePath);
                string FullfileName = Path.Combine(Program.taskFolderPath,"ExcelFileInfo.txt");
                ...
                using (FileStream fs = File.Create(FullfileName))
                {
                    ...
                    string arguments = " -u=" + Program.userName + " -p=" + Program.password + " -g=" + Program.group + " -i=" + Program.itemIDRecieved + " -r=" + Program.revIDReceived + " -dsName=" + Program.itemIDRecieved + " -dsType=MSExcelX -dsRelation=IMAN_specification -fileRef=excel -filePath=\"" + XLFiles[0] + "\" -log=" + Program.taskFolderPath + "\\3de_upload.log -dsNamedRefName=" + Program.itemIDRecieved + ".xlsx";
                    Byte[] excelfilePath = new UTF8Encoding(true).GetBytes(arguments);
                    fs.Write(excelfilePath, 0, excelfilePath.Length);
                }
            }
        }
        public static String getPropertyFilePath(String ReqdPropertyFile)
        {
            String ClickOnceLocation = GetClickOnceLocation();
            String PropertyFile = Path.Combine(ClickOnceLocation, ReqdPropertyFile);
            return PropertyFile;
        ...
        public static void UploadExcelTemplateToTeamcenterUsing3DEUpload(string logFilePath)
        {
            ...
            foreach (String XLFile in XLFiles)
            {
                ...
                Utility.Log("Enter UploadExcelTemplateToTeamcenterUsing3DEUpload ", logFilePath);

                string util = getPropertyFilePath("3DE_Upload_Dataset_TC11.exe");

                Utility.Log("Info : 3DE_Upload_Dataset_TC11 path:" + util, logFilePath);

                if (!File.Exists(util))
                {
                    Utility.Log("ERROR : 3DE_Upload_Dataset_TC11.exe file is missing.", logFilePath); // File not found exception

                    return;
                }

                string arguments = " -u=" + Program.userName + " -p=" + Program.password + " -g=" + Program.group + " -i=" + Program.itemIDRecieved + " -r=" + Program.revIDReceived + " -dsName=" + Program.itemIDRecieved + " -dsType=\"MS ExcelX\" -dsRelation=IMAN_specification -fileRef=excel -filePath=\"" + XLFiles[0] + "\" -log=" + Program.taskFolderPath + "_3de_upload.log -dsNamedRefName=" + Program.itemIDRecieved + ".xlsx";

                Utility.Log("Info : 3DE_Upload_Dataset_TC11  arguments:" + util + " " + arguments, logFilePath);
                ...
                string s = proces.StandardOutput.ReadToEnd();

                Utility.Log("INFO : 3DE_Upload_Dataset_TC11 info - " + s, logFilePath);
            }
        }

        /*        public static void uploadExcelTemplateToTeamcenter(string logFilePath)
                {
                   ...
                   foreach (String XLFile in XLFiles)
                    ...
                    objSEEC.ImportDocumentsToServer(1, ref ppsaExcelFileList, bStrUserName, Program.password, Program.group, Program.role, Program.URL, true, false, false, false, true, false, false, false, null);
                }
        */
