...
using SolidEdge.RevisionManager;
    ...
    class SolidEgeData2
    {
        ...
        public static void traverseAssembly1(String assemblyFileName, String logFilePath, TreeNode parentNode)
        {
            ...
            for (int i = 1; i <= linkDocuments.Count; i++)
            {
                ...
                {
                    Utlity.Log("SKYnameDictionary: is empty" + LinkDocumentName, logFilePath);
                    continue;
                }

                String SKYname = "";
                if (SKYnameDictionary.ContainsKey(LinkDocumentName) == true)
                {
                    SKYnameDictionary.TryGetValue(LinkDocumentName, out SKYname);
                }
                if (SKYname == null || SKYname.Equals("") == true)
                {
                    Utlity.Log("SKYname: is empty" + LinkDocumentName, logFilePath);
                    continue;
                }
                String item_name = "";
                if (SKYname.Contains(".") == true)
                {
                    ...
                    if (SKYnameArray == null || SKYnameArray.Length == 0)
                    {
                        Utlity.Log("SKYnameArray: is empty" + LinkDocumentName, logFilePath);
                        continue;
                    }
                    item_name = SKYnameArray[0];
                }
                t.Text = LinkDocumentName + ";" + item_name;
        ...
        public static void CopyAndReplaceSuffixForVariableParts2(String folderToPublish, String assemblyFileName, List<String> variableParts, String logFilePath)
        {
            ...
            if (application == null)
            {
                Utlity.Log("CopyAndReplaceSuffixForVariableParts2: application is Empty", logFilePath);
                return;
            ...
            {
                Utlity.Log("CopyAndReplaceSuffixForVariableParts2: assemblyDocument is Empty", logFilePath);
                return;
            ...
            {
                Utlity.Log("CopyAndReplaceSuffixForVariableParts2: linkedDocuments is Empty", logFilePath);
                return;
            ...
            {
                Utlity.Log("Close: " + "Exception: " + ex.Message, logFilePath);
            }
        ...
        public static void CopyAndReplaceSuffixForVariableParts3(String folderToPublish, String assemblyFileName, List<String> variableParts, String Suffix, String logFilePath)
        {
            ...
            {
                String AssemFileName = System.IO.Path.GetFileName(newAssemblyFilePath);
                if (variableParts.Contains(AssemFileName) == true)
                {

                    String fileNameWithoutExtn = System.IO.Path.GetFileNameWithoutExtension(AssemFileName);
                    String extn = System.IO.Path.GetExtension(AssemFileName);
                    ...
                    newAssemblyFilePath = newpartFileNameFullPath;
                }
            }

            if (System.IO.File.Exists(newAssemblyFilePath) == false)
            {
                Utlity.Log("Skipping REPLACE", logFilePath);
                return;
            }
        ...
        public static void CopyAndReplaceSuffixForVariableParts3(String folderToPublish, String assemblyFileName, List<String> variableParts,String Suffix, String logFilePath,String Option)
        {
            ...
            // 14 SEPT - FIX FOR ISSUE NO---1
            if (variableParts != null && variableParts.Count > 0)
            {
                String AssemFileName = System.IO.Path.GetFileName(assemblyDocument.FullName);
                if (variableParts.Contains(AssemFileName) == true)
                {
                    String linkedDocumentPath = assemblyDocument.AbsolutePath;
                    ...
                    if (Option.Equals("RENAME", StringComparison.OrdinalIgnoreCase) == true)
                    {
                        ...
                        assemblyDocument.Rename(newpartFileNameFullPath);
                    }
                    else
                    {
                        Utlity.Log(newpartFileNameFullPath, logFilePath);
                        assemblyDocument.Replace(newpartFileNameFullPath);
                    }
                }
            }
            ...
            for (int i = 1; i <= linkedDocuments.Count; i++)
            {
                ...
                if (variableParts != null && variableParts.Count > 0)
                {
                    if (variableParts.Contains(fileName) == true)
                    {
                        ...
                        if (Option.Equals("RENAME", StringComparison.OrdinalIgnoreCase) == true)
                        {
                            Utlity.Log("RENAME: " + newpartFileNameFullPath, logFilePath);
                            linkedDocument.Rename(newpartFileNameFullPath);
                        }
                        else
                        {
                            Utlity.Log("REPLACE: " + newpartFileNameFullPath, logFilePath);
