...
namespace DemoAddInTC.controller
{
    class SolidEdgeHighLighter
    {
        ...
        public static Dictionary<String, String> SKYnameDictionary = new Dictionary<string, string>();

        public static Dictionary<String, String> getSKYnameDictionary()
        {
            return SKYnameDictionary;
        }
            ...
            try
            {
                ...
                if (objAssemblyDocument != null)
                {
                   ...
                   // Utlity.Log("AssemDoc.Name : " + objAssemblyDocument.Name, logFilePath);                    
                    ...
                    ReadAndFillProperties(objAssemblyDocument, logFilePath);
                    for (int i = 1; i <= occurrences.Count; i++)
                    {
                        ...
                        if (occurrence.OccurrenceDocument is SolidEdgePart.PartDocument)
                        {
                            ...
                            ReadAndFillProperties(partDoc, logFilePath);
                        ...
                        else if (occurrence.OccurrenceDocument is SolidEdgePart.SheetMetalDocument)
                        {
                            ...
                            ReadAndFillProperties(sheetMetalDoc, logFilePath);                            
        ...
        private static void traverseAssembly(SolidEdgeAssembly.AssemblyDocument assemDoc, String logFilePath)
        {
            ...
            ReadAndFillProperties(assemDoc, logFilePath);
        ...
        //part
        private static void ReadAndFillProperties(SolidEdgePart.PartDocument partDoc, string logFilePath)
        {
            Utility.Log("ReadAndFillProperties: PartDocument", logFilePath);
            try
            {
                //object obj = partDoc.Properties;

                SolidEdgeFramework.PropertySets propSets = (SolidEdgeFramework.PropertySets)partDoc.Properties;
                if (propSets == null) return;

                foreach (SolidEdgeFramework.Properties properties in propSets)
                {
                    String propName = properties.Name;
                    //Utility.Log("ReadAndFillProperties: propName: " + propName, logFilePath);
                    //if (properties.Name.Equals("Custom", StringComparison.OrdinalIgnoreCase) == true)
                    //{
                    //    foreach (SolidEdgeFramework.Property pro in properties)
                    //    {
                    //        String name = pro.Name;
                    //        if (name.Equals("SKYname", StringComparison.OrdinalIgnoreCase) == true)
                    //        {
                    //            String type = pro.Type.ToString();
                    //            String value = (String)pro.get_Value();
                    //            String fullName = partDoc.FullName;
                    //            if (SKYnameDictionary.ContainsKey(System.IO.Path.GetFileName(fullName)) == false)
                    //            {
                    //                SKYnameDictionary.Add(System.IO.Path.GetFileName(fullName), value);
                    //            }
                    //            Utility.Log("ReadAndFillProperties: name: " + name, logFilePath);
                    //            Utility.Log("ReadAndFillProperties: value: " + value, logFilePath);
                    //            Utility.Log("ReadAndFillProperties: fullName: " + fullName, logFilePath);
                    //        }
                    //    }
                    ...
                    if (properties.Name.Equals("ProjectInformation", StringComparison.OrdinalIgnoreCase) == true)
                    {
                        foreach (SolidEdgeFramework.Property pro in properties)
                        {
                            String name = pro.Name;
                            //Utility.Log("ReadAndFillProperties: name: " + name, logFilePath);
                            if (name.Equals("Project Name", StringComparison.OrdinalIgnoreCase) == true)
                            {
                                String type = pro.Type.ToString();
                                String value = (String)pro.get_Value();
                                String fullName = partDoc.FullName;
        ...
        //sheetMetal
        private static void ReadAndFillProperties(SolidEdgePart.SheetMetalDocument sheetMetalDoc, string logFilePath)
        {
            Utility.Log("ReadAndFillProperties: SheetMetalDocument", logFilePath);
            try
            {
                //object obj = partDoc.Properties;

                SolidEdgeFramework.PropertySets propSets = (SolidEdgeFramework.PropertySets)sheetMetalDoc.Properties;
                if (propSets == null) return;
        ...
        //assembly
        private static void ReadAndFillProperties(SolidEdgeAssembly.AssemblyDocument assemblyDoc, string logFilePath)
        {
            Utility.Log("ReadAndFillProperties: AssemblyDocument", logFilePath);
            try
            {
                SolidEdgeFramework.PropertySets propSets = (SolidEdgeFramework.PropertySets)assemblyDoc.Properties;
                if (propSets == null) return;
                ...
                foreach (SolidEdgeFramework.Properties properties in propSets)
                {
                    ...
                    //            String type = pro.Type.ToString();
                    //            String value = (String)pro.get_Value();
                    //            String fullName = assemblyDoc.FullName;
                    //            if (SKYnameDictionary.ContainsKey(System.IO.Path.GetFileName(fullName)) == false)
                    //            {
                    //                SKYnameDictionary.Add(System.IO.Path.GetFileName(fullName), value);
                    //            }
