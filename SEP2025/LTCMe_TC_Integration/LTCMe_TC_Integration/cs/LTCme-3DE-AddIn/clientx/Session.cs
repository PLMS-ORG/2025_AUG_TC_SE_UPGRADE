...
namespace Teamcenter.ClientX
{
    ...
    public class Session
    {
         ...
         *
         */
        //private static Teamcenter.ClientX.AppXCredentialManager credentialManager;
         ...
         *
         */
        public User login(String user, String password,String group,String role,String discriminator,String logFilePath)
        {
            ...
            {
                DemoAddInTC.utils.Utility.Log("Unable to Login: " + "Session Service is NULL", logFilePath);
                return null;
            }
            String[] credentials = { user, password, group, role, discriminator };
            ...
            try
            {
               ...
               LoginResponse resp = sessionService.Login(credentials[0], credentials[1],
                ...
                if (resp.ServiceData.sizeOfPartialErrors() > 0)
                {
                    ...
                    DemoAddInTC.utils.Utility.Log("Exception while Logging into Teamcenter server", logFilePath);
                    for (int i = 0; i < resp.ServiceData.sizeOfPartialErrors(); i++)
                    {

                        for (int ii = 0; ii < resp.ServiceData.GetPartialError(i).Messages.Length; ii++)
                        {
                            String error = resp.ServiceData.GetPartialError(i).Messages[ii].ToString();                         

                            sb.Append(error + ",");
                        }
                    }

                    String eror = sb.ToString().TrimEnd(',');
                    DemoAddInTC.utils.Utility.Log("Exception while Logging : " + eror, logFilePath);
                    return null;
                }
                return resp.User;
            }
            catch (InvalidCredentialsException e)
            {
               // credentials = credentialManager.GetCredentials(e);
                DemoAddInTC.utils.Utility.Log("Unable to Login: " + e.Message, logFilePath);
            ...
            {
                DemoAddInTC.utils.Utility.Log("Unable to Login: " + ex.Message, logFilePath);
                DemoAddInTC.utils.Utility.Log("Unable to Login: " + ex.StackTrace, logFilePath);
            }
            return null;
